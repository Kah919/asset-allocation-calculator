...
describe('validateCalculatorInput', () => {
  ...
  it('rejects empty USD', () => {
    expect(validateCalculatorInput({ ...valid, totalUsd: '' }))
      .toBe('Please enter a valid USD amount.')
  })

  it('rejects non-numeric USD', () => {
    expect(validateCalculatorInput({ ...valid, totalUsd: 'abc' }))
      .toBe('Please enter a valid USD amount.')
  })
  ...
  it('rejects missing percentages', () => {
    expect(validateCalculatorInput({ ...valid, percentA: '' }))
      .toBe('Both percentage fields must be filled in.')
    expect(validateCalculatorInput({ ...valid, percentB: '' }))
      .toBe('Both percentage fields must be filled in.')
  })
